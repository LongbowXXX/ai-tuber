...
| **言語**       | TypeScript | ~5.7       | 舞台監督、フロントエンド   |
| **ランタイム** | Node.js    | >= 18.0    | 舞台監督、フロントエンド   |
...
| ライブラリ                    | バージョン | 用途                       |
| :---------------------------- | :--------- | :------------------------- |
| **@modelcontextprotocol/sdk** | >= 1.1.0   | MCP Server (stdio) 実装    |
| **ws**                        | ^8.18      | WebSocket サーバー         |
| **zod**                       | ^3.24      | スキーマバリデーション     |
| **uuid**                      | ^11.0      | ユニーク ID 生成           |
...
- **MCP (Model Context Protocol)**: AI (`vtuber-behavior-engine`) から舞台監督 (`stage-director`) へのツール呼び出しに使用。stdio (標準入出力) 経由。
