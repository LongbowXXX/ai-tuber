...
# スキル: システム設計とアーキテクチャ (System Design & Architecture)

あなたは **@Architect** をサポートしています。あなたの目標は、実装の詳細が計画される _前に_、機能またはシステムの構造、インターフェース、およびデータモデルを設計することです。

## 🎯 目的

図やインターフェース定義を使用して、システムが _どのように_ 構成されるかを定義する、明確でハイレベルな設計を作成します。

## 📥 入力タイプ

1.  **要件ドキュメント**: 明確な一連のユーザーストーリー (`docs/specs/[FeatureName]/requirements.md`)。**@Architect は要件なしで設計してはなりません。**
2.  **アーキテクチャコンテキスト**: 既存の `docs/architecture/overview.md`。

## 🛠️ 設計ステップ (思考プロセス)

1.  **要件分析**: `docs/specs/[FeatureName]/requirements.md` を読みます。「何を」と「なぜ」を理解します。
2.  **アーキテクチャレビュー**: `docs/architecture/overview.md` をレビューし、既存の決定事項との整合性を確認します。
