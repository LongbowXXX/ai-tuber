...
2.  **変更の分析**: 最後のタグ以降のコミットをチェックします (`git log <last_tag>..HEAD --oneline`)。**重要**: コミットメッセージは誤解を招く可能性があるため、`git diff <last_tag>..HEAD --stat` および `git diff <last_tag>..HEAD` を使用して実際のファイル変更も確認し、内容を検証してください。
3.  **インクリメントの決定**:
    - **メジャー**: 破壊的変更。
    - **マイナー**: 新機能 (後方互換性あり)。
    - **パッチ**: バグ修正。
