...
2. **自動推論**: 目標から以下の項目を自動的に決定し、提案として提示します。
   ...
   - **エージェント**:
     ...
     - **重要**: Terraform 化されたプロジェクトでは、可能な限り汎用的な `agent` ではなく、役割固有のエージェント（例: `@Developer`, `@Architect`）を使用することをお勧めします。
   - **ツール**:
     - 基本的に未指定（すべてのツールが有効）。
     - セキュリティ上の理由などで制限が必要な場合のみ明示的に指定。
   - **出力形式**: 目標から自然に導き出される形式を想定。
   - **スコープ**:
...
1. **YAML フロントマター**:
   ...
   - `agent`: 推論されたエージェントを設定します（理由を説明します）。
   - `tools`: 基本的に除外（すべてのツールが有効）。制限が必要な場合のみ明示的に指定。
   - `model`: 特別な要件がない限り除外（デフォルトモデルを使用）。
...
- 達成したいことと期待される出力形式を明確に記述する。
- 期待される入力と出力の例を提供する。
- 重複を避け、カスタム指示への Markdown リンクを活用する。
- 公式ドキュメントに記載されている変数構文を正確に使用する。
- 具体的な実行可能な指示を書く。
- YAML フロントマターの形式を厳守する。

## ステップ 4: 提案とレビュー

**最小限の確認で完了させます。**

1. **完成したプロンプトファイルを提示する**:
