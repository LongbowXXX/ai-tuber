...
「何かが壊れている」状態から、「なぜ壊れているのか、どう修正すればよいのか」を正確に把握した状態へ移行します。
...
1.  **観察**: 症状は何ですか？ (エラーメッセージ、誤った出力、クラッシュ)
2.  **仮説**: 何が原因と考えられますか？ (Null ポインター、ロジックエラー、API の変更)
3.  **実験 (再現)**:
    - スクリプトでこれを再現できますか？
...
`docs/specs/fixes/[IssueID]/` に **バグ修正計画** ファイルを出力する必要があります。
