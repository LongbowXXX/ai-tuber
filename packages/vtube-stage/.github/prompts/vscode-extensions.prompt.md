...
1.  **ドキュメントの取得**: `https://code.visualstudio.com/docs/editing/workspaces/multi-root-workspaces#_extension-recommendations` を読み込みます。
2.  **技術スタックの分析**: フレームワーク、言語、およびツールを特定します。
3.  **反復的な設定**: 拡張機能を一つずつ提案します。
...
リスト全体をすぐに生成**しないでください**。ユーザーが判断できるように、拡張機能を**一つずつ**（または関連するペアで）提案する必要があります。

1.  **拡張機能の選択**: 優先度の高い拡張機能を選択します（例: `dbaeumer.vscode-eslint`）。
2.  **提案**: 価値を説明します。「 `package.json` で ESLint が使用されているのを見つけました。推奨事項に `VS Code ESLint` 拡張機能を追加すべきですか？」
3.  **待機**: ユーザーの確認を待ちます。
4.  **繰り返す**: 次の拡張機能に進みます。
