...
description: プロジェクトの新しいバージョンをリリースします (SemVer, Changelog, Tags)。
agent: Librarian
---
...
2.  **変更の分析**: 最後のタグ以降のコミットをチェックします（ `git log <last_tag>..HEAD --oneline` ）。**重要**: コミットメッセージは誤解を招く可能性があるため、 `git diff <last_tag>..HEAD --stat` および `git diff <last_tag>..HEAD` を使用して実際のファイルの変更も確認し、内容を検証してください。
3.  **インクリメントの決定**:
    - **Major**: 破壊的変更。
    - **Minor**: 新機能（後方互換性あり）。
    - **Patch**: バグ修正。
...
- **CHANGELOG.md**: 新しいバージョンエントリで更新されます。
- **Git Tag**: リポジトリに新しいタグが作成されます。
