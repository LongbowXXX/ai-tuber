...
MCP (Model Context Protocol) Client として `vtube-stage` 内蔵の MCP サーバーに接続し、生成した対話やアクションを実際のキャラクター動作（発話、アニメーション、表示）として実行します。
...
2. **MCP Client**: `vtube-stage` 内蔵の MCP サーバーに SSE で接続（既定 `http://localhost:8080/sse`）し、以下のツールを使用してキャラクターを制御します。
   - `speak`: 発話と感情表現
...
# vtube-stage 内蔵 MCP サーバーへの接続先 (環境変数名は歴史的経緯によるもの)
