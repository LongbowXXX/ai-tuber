...
MCP (Model Context Protocol) Client として `vtube-stage` に接続し、生成した対話やアクションを実際のキャラクター動作（発話、アニメーション、表示）として実行します。
...
2. **MCP Client**: `vtube-stage` の MCP Server (`http://localhost:8080/sse`) に HTTP/SSE で接続し、以下のツールを使用してキャラクターを制御します。
   - `speak`: 発話と感情表現
...
# VTube Stage MCP Server URL (Electron app provides MCP server)
STAGE_DIRECTOR_MCP_SERVER_URL=http://localhost:8080/sse
```
