...
| Framework | Electron                    | -             | vtube-stage デスクトップアプリ               |
| Protocol  | MCP (HTTP/SSE)              | >= 1.19.0     | AI→Stage のツール呼び出し                    |
...
- **MCP サーバー統合（HTTP/SSE）**: Stage が MCP サーバーを内蔵し、HTTP/SSE で AI からのツール呼び出しを受信。
- **Electron 統合**: Stage は Electron アプリとして、GUI ウィンドウ（1920x1080）と MCP サーバーを同時提供。
...
│   └── vtube-stage/            # Electron App (MCP Server + React + Three.js + VRM Renderer)
├── docs/                       # プロジェクトドキュメント
...
| --------------------------------- | ------------------------------ | ---------------------------------------------------------- |
| `packages/vtuber-behavior-engine` | AI コア（ADK/MCP Client）      | `src/vtuber_behavior_engine/main.py`, `agent_runner.py`   |
| `packages/vtube-stage`            | Electron App（MCP Server/描画）| `src/main.tsx`, `src/mcp/server.ts`, `src/electron/main.ts` |
...
| **VTube Stage**        | MCP サーバー + VRM/TTS/表示を実行する Electron App | `packages/vtube-stage`                                |
| **MCP Tool**           | AI から舞台を操作する関数インターフェース         | `speak`, `trigger_animation`, `display_markdown_text` |
| **MCP Command**        | AI から Stage へ送られる JSON コマンド            | `speak`, `triggerAnimation`, `displayMarkdown`        |
...
| MCP Server           | `packages/vtube-stage/src/mcp/server.ts`                                 | HTTP/SSE エンドポイント（localhost:8080/sse）|
| Frontend Main        | `packages/vtube-stage/src/main.tsx`                                      | React のルートをマウント（1920x1080）        |
...
- MCP コマンド（`command`/`payload` 形状）を片側だけ変更すること。
- `speakEnd` を送らないまま `speak` 同期を前提とした機能を実装すること。

### Patterns to Use

- `vtube-stage` の MCP ツール追加は「ツール定義 → ハンドラー実装 → 完了通知」という既存パターンに合わせる。
- `vtube-stage` のコマンド処理は「型定義 → validator registry → handler switch」という既存パターンに合わせる。
...
# vtube-stage (Electron アプリとして起動)
npm install
npm run dev

# vtuber-behavior-engine (MCP クライアントとして vtube-stage に接続)
uv sync --extra dev
